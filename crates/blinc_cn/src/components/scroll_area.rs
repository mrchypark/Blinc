...
impl ScrollAreaBuilder {
    ...
    ///
    /// - `false` (default): legacy fallback size (`300x400`) when width/height are not set.
    /// - `true`: width/height follow parent when not explicitly set.
...
mod tests {
    ...
    #[test]
    fn test_scroll_area_is_non_responsive_by_default() {
        init_theme();
        let builder = scroll_area();
        let config = builder.config.borrow();
