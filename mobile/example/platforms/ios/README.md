...
A minimal iOS application demonstrating Blinc UI framework integration with Metal rendering.
...
From the `mobile/example` directory:
...
2. **Static Library** (`libblinc_app.a`) contains the compiled Rust code
...
To customize the UI, you need to:

1. Create a Rust library with your UI code
2. Export a UI builder function via C FFI
3. Register it with `blinc_set_ui_builder()`
...
#[no_mangle]
pub extern "C" fn my_app_ui_builder(ctx: *mut WindowedContext) {
    if ctx.is_null() { return; }
    let ctx = unsafe { &mut *ctx };

    // Build your UI here using ctx.width, ctx.height, etc.
}
```

## Troubleshooting

### "Library not found: -lblinc_app"
